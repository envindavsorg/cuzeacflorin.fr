		...
		<Panel>
			<div className="relative">
				...
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
					{SOCIAL_LINKS.map(({ title, description, href, icon, handle }) => {
						const config =
							FOLLOWERS_CONFIG[title as keyof typeof FOLLOWERS_CONFIG];
						const { step, label } = config;
						const count = config ? data[config.key as keyof typeof data] : 0;
